...
class MoCo(nn.Module):
    ...
    """
    def __init__(self, base_encoder, dim=128, K=65536, m=0.999, T=0.07, mlp=False, dist = False):
        """
        ...
        if dist:
            self._dequeue_and_enqueue = self._dequeue_and_enqueue_ddp
            self._batch_shuffle = self._batch_shuffle_ddp
            self._batch_unshuffle = self._batch_unshuffle_ddp
        else:
            self._dequeue_and_enqueue = self._dequeue_and_enqueue_single_gpu
