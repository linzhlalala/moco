...
parser.add_argument('--dist-url', default=None, type=str, #'tcp://224.66.41.62:23456'
                    help='url used to set up distributed training')
parser.add_argument('--dist-backend', default=None, type=str, #'nccl'
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = moco.builder.MoCo(
        ...
        args.moco_dim, args.moco_k, args.moco_m, args.moco_t, args.mlp, args.distributed)
    #print(model)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        acc1, acc5 = accuracy(output, target, topk=(1, 5)) #<-meter
        losses.update(loss.item(), images[0].size(0))
        top1.update(acc1[0], images[0].size(0)) # <-to meter
...
def accuracy(output, target, topk=(1,)):
    ...
    with torch.no_grad():
        ...
        # value, indices = output.topk(....)
        # will find to topk in each batch
